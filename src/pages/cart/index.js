...
const Cart = () => {
    ...
    const deleteItem = async (id) => {
        const response = await axios.delete(`http://localhost:2000/cartItems/${id}`);
        if (response !== null) {
            getCartData("http://localhost:2000/cartItems");
    ...
    const emptyCart = () => {
        ...
        cartItems.length !== 0 &&
            cartItems.map((item) => {
                response = axios.delete(`http://localhost:2000/cartItems/${parseInt(item.id)}`);
            })
        if (response !== null) {
            getCartData("http://localhost:2000/cartItems");
        }
                    ...
                    <div className='row'>
                        <div className='col-md-8'>
                            <div className='d-flex align-items-center w-100'>
                                ...
                                <span className='paddingClearCart clearCart d-flex align-items-center cursor mt-5'
                        ...
                        <div className='col-md-4 cartRightBox'>
                            <div className='card p-4 '>
                                <div className='d-flex align-items-center mb-4'>
                                    ...
                                    <h3 className='ml-auto mb-0 font-weight-bold'><span className='text-g paddingLeftCart'>
                                    {
                                            cartItems.length !== 0 &&
                                ...
                                <div className='d-flex align-items-center mb-4'>
                                    ...
                                    <h3 className='ml-auto mb-0 font-weight-bold paddingLeftCart'><span >Free</span></h3>
                                </div>


                                <div className='d-flex align-items-center mb-4'>
                                    <h5 className='mb-0 text-light'>Estimate for</h5>
                                    <h3 className='ml-auto mb-0 font-weight-bold paddingLeftCart'>United Kingdom</h3>
                                </div>


                                <div className='d-flex align-items-center mb-4'>
                                    <h5 className='mb-0 text-light'>Total</h5>
                                    <h3 className='mb-0 font-weight-bold paddingLeftCart'><span className='text-g'>
                                    {
                                            // cartItems.length !== 0 &&
                                            data.cartItems.length !== 0 &&
                                            // cartItems.map(item => parseInt(item.price.split(",").join("")) * item.quantity).reduce((total, value) => total + value, 0)
                                            data.cartItems.map(item => parseInt(item.price.split(",").join("")) * item.quantity).reduce((total, value) => total + value, 0)
                                        }
                                    </span></h3>
                                </div>
